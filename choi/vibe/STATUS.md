...
- 上の2フィールド(`data.deck.part_id`・`data.unit.course_id`)は教材では
  ないが、DATA.md §3.3 が識別のために読んでよいフィールドとして列挙して
  いる。本書の用途はその範囲内である。いずれも null がありうるため、キーの
  存在チェックと null チェックを介して読む(DATA.md §4 と同じ)
